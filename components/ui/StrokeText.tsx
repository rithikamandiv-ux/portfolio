/**
 * StrokeText - vendored from React Bits (TextAnimations/StrokeText, TS + Tailwind variant).
 * https://reactbits.dev/text-animations/stroke-text
 *
 * Copyright (c) 2026 David Haz. MIT + Commons Clause License Condition v1.0.
 *
 * LOCAL MODIFICATION: added an optional `onComplete` callback so the splash screen can
 * dismiss when the animation actually finishes, instead of duplicating the internal
 * duration formula in a separate timer. Three places are patched, all marked with
 * "LOCAL MOD" comments below. Re-apply this patch if you update from upstream.
 */
...
export interface StrokeTextProps {
  ...
  className?: string;
  style?: CSSProperties;
  /** LOCAL MOD: fires once the draw + fill timeline finishes, or immediately under reduced motion. */
  onComplete?: () => void;
}
...
const StrokeText = ({
  ...
  // LOCAL MOD: hold the callback in a ref so the animation effect can call the latest version
  // without listing `onComplete` in its dependency array. If it were a dependency and the parent
  // passed an inline arrow function, every parent render would create a new function identity,
  // tearing down and rebuilding the GSAP timeline and restarting the animation mid-draw.
  ...
  useEffect(() => {
    ...
    if (prefersReducedMotion) {
      setEnd();
      // LOCAL MOD: this branch returns before any timeline is built, so without this call
      // onComplete would never fire and a consumer waiting on it (the splash screen) would
      // stay mounted forever for reduced-motion users.
    ...
    const build = () => {
      ...
      const tl = gsap.timeline({
        ...
        // LOCAL MOD: note this never fires when trigger === 'loop', since an infinitely
        // repeating timeline has no completion.
